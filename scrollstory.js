...
function scrollStory(options) {
  var pluginName = "scrollStory";
  var eventNameSpace = "." + pluginName;
  var defaults = {
    // selector or node where we launch scrollStory
    container: null,
    ...
    itementerviewport: function () {},
    itemexitviewport: function () {},
    categoryfocus: function () {},
    categeryblur: function () {},
  ...
  /**
   * Simpler isPlainObject
   * this probably needs lodash…
  ...
  /**
   * Deep extend
   * equivalent to $.extend(true, {}, objA, objB);
  ...
  var trigger = function (el, event, data) {
    if (window.CustomEvent && typeof window.CustomEvent === "function") {
      var triggerEvent = new CustomEvent(event, { detail: data });
    } else {
      var triggerEvent = document.createEvent("CustomEvent");
      triggerEvent.initCustomEvent(event, true, true, data);
    }

    el.dispatchEvent(triggerEvent);
  };

  var winHeight = window.innerHeight; // cached. updated via _handleResize()
  ...
  // based on https://github.com/russellgoldenberg/scrollama/blob/master/src/dom.js
  var select = function (selector, parent = document) {
  ...
  // based on https://github.com/russellgoldenberg/scrollama/blob/master/src/dom.js
  var selectAll = function (selector, parent = document) {
    if (typeof selector === "string") {
  ...
  function ScrollStory(options) {
    ...
    this.el = select(this.options.container);
  ...
  ScrollStory.prototype = {
    init: function () {
       ...
       */
      this.el.addEventListener("setup", this._onSetup.bind(this));
      this.el.addEventListener("destroy", this._onDestroy.bind(this));
      ...
      this.el.addEventListener("itemblur", this._onItemBlur.bind(this));
      this.el.addEventListener("itemfocus", this._onItemFocus.bind(this));
      this.el.addEventListener(
        "itementerviewport",
      ...
      this.el.addEventListener("itemfilter", this._onItemFilter.bind(this));
      this.el.addEventListener("itemunfilter", this._onItemUnfilter.bind(this));
      this.el.addEventListener(
        "categoryfocus",
      ...
      this.$trigger.className = pluginName + "Trigger";
      this.$trigger.id = pluginName + "Trigger-" + this._instanceId;

      this.$trigger.style.position = "fixed";
      this.$trigger.style.width = "100%";
      this.$trigger.style.height = "1px";
      this.$trigger.style.top = offsetToPx(this.options.triggerOffset) + "px";
      this.$trigger.style.left = "0px";
      this.$trigger.style.backgroundColor = "#ff0000";
      this.$trigger.style.transform = "translateZ(0)";
      this.$trigger.style.WebkitBackfaceVisibility = "hidden";
      this.$trigger.style.zIndex = 100;
      ...
      if (this.useNativeScroll) {
        ...
        if (typeof this.options.scrollEvent === "function") {
          ...
          window.addEventListener(this.options.scrollEvent, function () {
            scrollHandler();
          });
        }
      }
      ...
      window.addEventListener("DOMContentLoaded", resizeThrottle.bind(this));
      window.addEventListener("load", resizeThrottle.bind(this));
      window.addEventListener("resize", resizeThrottle.bind(this));
    ...
    filterAll: function (callback) {
      callback = isFunction(callback) ? callback.bind(this) : function () {};
    ...
    _scrollToItem: function (item, opts, callback) {
       ...
       */
      // VANILLA_FIXME: remove speed and easing?
      opts = deepExtend(
        {
      ...
      // VANILLA_FIXME: enable/disable behaviour smooth?
      var offsetTop = item.el.getBoundingClientRect().top + window.scrollY;
      var scrolllTop = offsetTop - offsetToPx(opts.scrollOffset);
      window.scrollTo({ top: scrolllTop, left: 0, behavior: "smooth" });

      // VANILLA_FIXME: there's no easy way to do a callback here :/
      // maybe with intersection observer?
      // https://stackoverflow.com/questions/46795955/how-to-know-scroll-to-element-is-done-in-javascript
      window.setTimeout(debouncedCallback, 100);
    },
    ...
    updateOffsets: function () {
      ...
      for (i = 0; i < length; i++) {
        item = items[i];
        box = item.el.getBoundingClientRect();
    ...
    _updateScrollPositions: function () {
      ...
      if (this._distanceToOffset < 0) {
        percentScrollToLastItem =
          1 - lastItem.distanceToOffset / (this._height - lastItem.height);
        percentScrollToLastItem =
    ...
    addItems: function (items, opts) {
      opts = deepExtend({ handleRepaint: true }, opts);

      if (typeof items === "string") {
        this._prepItemsFromSelection(this.el.querySelectorAll(items));
        ...
        this._prepItemsFromSelection(
          this.el.querySelectorAll(this.options.contentSelector)
        );
      }
    ...
    destroy: function (removeMarkup) {
      ...
      this.el.setAttribute("data-plugin_" + pluginName, null);
    ...
    _onDestroy: function () {
      // remove events
      this.el.removeEventListener(eventNameSpace);
      window.removeEventListener(eventNameSpace);

      // item classes
      // VANILLA_FIXME: should we use es6?
      ...
      itemClassesToRemove.forEach((d) => item.el.classList.remove(d));

      // container classes
      // VANILLA_FIXME: should we use es6?
      var classesToRemove = [...this.el.classList].filter(
        (d) => d.lastIndexOf(pluginName, 0) === 0
      );
      classesToRemove.forEach((c) => this.el.classList.remove(c));

      this.$trigger.parentNode.removeChild(this.$trigger);
    },

    _onContainerActive: function () {
      this.el.classList.add(pluginName + "Active");
    },
    ...
    _onItemEnterViewport: function (e) {
      e.detail.el.classList.add("inviewport");
    },

    _onItemExitViewport: function (e) {
      e.detail.el.classList.remove("inviewport");
    },
    ...
    _onTriggerOffsetUpdate: function (ev, offset) {
      this.$trigger.style.top = offset + "px";
    },
    ...
    _manageContainerClasses: function (prefix, value) {
      // VANILLA_FIXME: should we use es6?
      var classesToRemove = [...this.el.classList].filter(
        (d) => d.lastIndexOf(prefix, 0) === 0
      );
      classesToRemove.forEach((c) => this.el.classList.remove(c));
      this.el.classList.add(prefix + value);
    },
    ...
    _addItem: function (data, el) {
      ...
      var item = {
        ...
        data: Object.assign({}, data, domData),
      ...
      el.classList.add("scrollStoryItem");
    ...
    _trigger: function (eventType, event, data) {
      ...
      if (isFunction(callback)) {
        ...
        // VANILLA_FIXME: not sure what this does
        // can i comment?
        // event = $.Event(event);
        event = {};
        ...
        // fire event
        trigger(event.target, eventType, data);
  ...
  return selectAll(options.container).forEach(function (node) {
    if (!node.__scrollStory__) {
      node.__scrollStory__ = true;
      return new ScrollStory(options);
    }
  });
}
